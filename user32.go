...
func ShowCursor(show bool) int {
	ret, _, _ := procShowCursor.Call(
		uintptr(BoolToBOOL(show)),
	)
	return int(ret)
}

func CreateCursor(hInst HINSTANCE, xHotSpot, yHotSpot, nWidth, nHeight int, pvANDPlane, pvXORPlane unsafe.Pointer) HCURSOR {
...
func SendInput(input INPUT) (uint32, uint32, error) {
	...
	ret, ret1, e := procSendInput.Call(1, uintptr(unsafe.Pointer(&input)), unsafe.Sizeof(input))

	return uint32(ret), uint32(ret1), e
}
